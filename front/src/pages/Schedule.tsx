              ...
              <div className="grid grid-cols-6 gap-2 min-w-[800px]">
                ...
                {timeSlots.map((timeSlot) => (
                  <React.Fragment key={timeSlot}>
                    <div className="p-3 text-sm text-center bg-muted/50 font-medium">
                      {timeSlot}
                    </div>
                    {daysOfWeek.map((day) => {
                      ...
                      return (
                        <div
                          key={`${day}-${timeSlot}`}
                          className="p-2 min-h-[80px] border border-border"
                        >
                          {course && (
                            <motion.div
          ...
          <CardContent>
            <div className="flex flex-wrap gap-3">
              {Object.entries(subjectColors).map(([subject, colorClass]) => (
                <Badge key={subject} variant="outline" className={colorClass}>
                  {subject}
                </Badge>
              ))}
            </div>
